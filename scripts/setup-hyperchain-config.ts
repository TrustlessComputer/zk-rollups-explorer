...
const buildAppConfig = (zkSyncEnvs: { [key: string]: string }) => {
  ...
  const verificationApiUrl = `https://verification.${domain}`;
...
const buildApiConfig = (zkSyncEnvs: { [key: string]: string }) => {
  ...
  return {
    ...
    CONTRACT_VERIFICATION_API_URL:
      zkSyncEnvs.API_CONTRACT_VERIFICATION_URL || '',
  };
};

const buildEnvFileContent = (json: { [key: string]: string | number }) =>
  Object.keys(json)
    .map((envName) => `${envName}=${json[envName]}`)
    .join('\n');
...
(async () => {
  ...
  if (!envFiles.length) {
    console.error(
      'No environment files found in your zksync-era repo. Please set up your hyperchain first.',
    );
    process.exit(1);
  }
  ...
  writeFileSync(
    path.join(__dirname, '../packages/app/src/configs/hyperchain.config.json'),
    JSON.stringify(appConfig, null, 2),
  );
  console.log('Updated app config at app/src/configs/hyperchain.config.json');

  writeFileSync(
    path.join(__dirname, '../packages/data-fetcher/.env'),
    buildEnvFileContent(dataFetcherConfig),
  );
  console.log('Updated data-fetcher env file at data-fetcher/.env');

  writeFileSync(
    path.join(__dirname, '../packages/worker/.env'),
    buildEnvFileContent(workerConfig),
  );
  console.log('Updated worker env file at worker/.env');

  writeFileSync(
    path.join(__dirname, '../packages/api/.env'),
    buildEnvFileContent(apiConfig),
  );
